...
const changeHtmlAccordingToChatWidgetDetails = async () => {
  const chatWidgetDetails = await fetch(
    "https://dondygroup.herokuapp.com/api/WhatsAppWidgetsView/" +
    window.Shopify?.shop,
    {
  ...
  let encodedURL = "";
  if (chatWidgetDetails.AddPageURL) {
    encodedURL = `${encodedCurrentURL}`;
  }
  let prefilledMessage = chatWidgetDetails.prefilledMessage || "";
  if (prefilledMessage != "" && encodedURL != "") {
    whatsAppLinkHtmlTag.setAttribute(
      "href",
      `https://api.whatsapp.com/send/?phone=85296609069&text=我想預約參觀BlueCoast&type=phone_number&app_absent=0`
    );
  } else {
    whatsAppLinkHtmlTag.setAttribute(
      "href",
  ...
  let getWidgetStyleBySize = (size) =>
    `style="--whatsapp-link-width: ${size}; --whatsapp-link-height: ${size};"`;
  let WidgetSizeStyle = getWidgetStyleBySize("47px");
  if (chatWidgetDetails?.WidgetSize) {
    WidgetSizeStyle = getWidgetStyleBySize(chatWidgetDetails?.WidgetSize);
  }
